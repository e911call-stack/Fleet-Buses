...
export default function SuperAdminDashboard() {
    ...
    try {
      ...
      if (tenantsData) {
        ...
        setStats({
          ...
          expiredTenants: tenantsData.filter(
            (t) => t.subscription_status === 'expired' || t.subscription_status === 'cancelled'
          ).length,
  ...
  return (
    <>
      <Head>
        <title>Super Admin – FleetGuard Platform</title>
      </Head>
        ...
        <main className="ml-60 p-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                Real-time view of all tenants and fleet activity
              </p>
            </div>
            <button
              onClick={fetchData}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              {
                label: 'Basic Plans',
                value: stats?.planBreakdown?.basic ?? 0,
                icon: CreditCard,
                color: 'text-gray-400',
                bg: 'bg-gray-800',
              },
              {
                label: 'Pro Plans',
                value: stats?.planBreakdown?.pro ?? 0,
                icon: TrendingUp,
                color: 'text-blue-400',
                bg: 'bg-blue-900/30',
              },
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    {['School', 'Slug', 'Plan', 'Status', 'Trial ends', 'Joined', 'Actions'].map(
                      (h) => (
                        <th
                          key={h}
                        ...
                        <td className="px-5 py-3.5">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                              STATUS_COLORS[tenant.subscription_status] ||
